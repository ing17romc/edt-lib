export const jsonToArray = json => {
	const result = [];
	for (const i in json) result.push(json[i]);
	return result;
};

export const getOptionsSelector = json => {
	const result = [];
	for (const i in json) result.push({ key: json[i], value: json[i] });
	return result;
};
...
export const lowerCaseText = value => getInitialValue(value, '').toLowerCase();

export const upperCaseText = value => getInitialValue(value, '').toUpperCase();

export const capitalText = value =>
	getInitialValue(value, '').replace(/\w\S*/g, w =>
