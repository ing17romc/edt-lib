...
const RadioButton = ({ id, name, label, disabled, required, readOnly, value, eventChange, ref = null }) => {
  ...
  return <div className={`radiobutton ${style()}`}  >
    <input type='radio'
      id={id}
      name={name}
      ...
      value={value}
      checked={id === value}
      onChange={(e) => { if (!readOnly) { eventChange(e) } }}
      ref={ref}
    />
    <label htmlFor={id}>
      {label}
