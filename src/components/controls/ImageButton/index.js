...
import { STYLE_STATUS_CONTROL, SIZE_CONTROL, ICONS } from '../../utils/constant';
...
const ImageButton = ({ id, text, icon, size = SIZE_CONTROL.MD, disabled, onClick }) => {

    const style = () => {
        return `imageButton ${ternaryOperation(disabled, STYLE_STATUS_CONTROL.DISABLED, '')}`;
    };

    return <abbr title={text}>
