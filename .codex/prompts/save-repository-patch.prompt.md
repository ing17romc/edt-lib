---
mode: 'agent'
description: 'Publish a patch version of the package. Commits+pushes, prepares the release, pauses for a manual npm publish, then resumes Storybook, changelog, and tag push.'
---
...
3. **Create or resume the patch version**:
   - If no release bump exists yet, run `npm version patch`.
   - If the repo is already bumped from an interrupted publish, reuse the existing version and do not run `npm version patch` again.
